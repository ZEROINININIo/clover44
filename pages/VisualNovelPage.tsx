...
const CharacterSprite = ({ id, isActive, emotion, vnTheme }: { id: string, isActive: boolean, emotion?: string, vnTheme: VNTheme }) => {
    const theme = getCharacterTheme(id, vnTheme);
...
const VisualNovelPage: React.FC<VisualNovelPageProps> = ({ chapter, onNextChapter, onPrevChapter, onExit, language }) => {
  ...
  const themeStyles = getThemeStyles(vnTheme);
  ...
  const parseRichText = (text: string) => {
    ...
    return parts.map((part, index) => {
        ...
        if (part.startsWith('[[GLITCH_GREEN::') && part.endsWith(']]')) {
            ...
            const shadow = 'drop-shadow-[0_0_10px_rgba(52,211,153,0.8)]';
            return (
        ...
        if (part.startsWith('[[GREEN::') && part.endsWith(']]')) {
            ...
            return createSpan(content, 'text-emerald-500', 'font-mono font-bold tracking-wide');
        }
        if (part.startsWith('[[VOID::') && part.endsWith(']]')) {
            const content = part.slice(8, -2);
            const color = 'text-fuchsia-500';
            return (
        ...
        if (part.startsWith('[[BLUE::') && part.endsWith(']]')) {
            ...
            return createSpan(content, 'text-blue-400', 'font-bold px-1');
        }
        if (part.startsWith('[[WHITE::') && part.endsWith(']]')) {
            const content = part.slice(9, -2);
            // In light theme, WHITE text needs to be black or dark
            const color = 'text-white font-bold drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]';
            return createSpan(content, color, 'animate-pulse');
        }
        if (part.startsWith('[[VOID_VISION::') && part.endsWith(']]')) {
            const content = part.slice(15, -2);
            const color = 'text-fuchsia-300 bg-fuchsia-950/50 border-fuchsia-500/50';
            return (
  ...
  const speakerTheme = getCharacterTheme(activeSpeakerId || 'unknown', vnTheme);
    ...
    <div className={`relative w-full h-full flex flex-col overflow-hidden font-mono select-none ${themeStyles.bg}`}>
            ...
            {currentNode.type === 'image' && (
                <div className={`relative max-w-[90%] max-h-[60%] border-4 border-current p-2 shadow-hard animate-zoom-in-fast ${vnTheme === 'bw' ? 'bg-black grayscale' : 'bg-black/20'}`}>
                    ...
                    {(() => {
                        ...
                        return (
                            <>
                                <img src={src} alt="Scene" className="max-h-[50vh] object-contain block select-none" onContextMenu={(e) => e.preventDefault()} draggable={false} />
                                {caption && (
            ...
            <div className="max-w-4xl mx-auto w-full flex flex-col gap-4">
                ...
                <div 
                    className={`
                        ...
                        ${currentNode.type === 'dialogue' ? `${speakerTheme.border} ${speakerTheme.bg} ${speakerTheme.text}` : 'bg-ash-dark/30 border-ash-gray/20'}
        ...
        {showEndModal && (
            <div className="absolute inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" onClick={(e) => e.stopPropagation()}>
                <div className={`w-full max-w-md border-2 p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col gap-6 relative bg-ash-black border-ash-light`}>
                    <div className="text-center space-y-2">
                        <CheckCircle size={48} className={`mx-auto mb-4 text-ash-light`} />
                        <h2 className="text-xl font-black uppercase tracking-widest">
                            {language === 'en' ? 'CHAPTER COMPLETE' : '本章閱讀完畢'}
                        </h2>
                        <div className={`text-xs font-mono border-b pb-4 mb-4 border-ash-gray/30 text-ash-gray`}>
                            {t.title}
                        </div>
                        <p className={`text-sm font-mono text-ash-gray`}>
                    ...
                    <div className="flex flex-col gap-3">
                        <button 
                            ...
                            className={`w-full py-4 border-2 font-black uppercase tracking-widest transition-all hover:scale-[1.02] flex items-center justify-center gap-2 bg-ash-light text-ash-black border-ash-light hover:bg-white`}
                        >
                            {language === 'en' ? 'NEXT CHAPTER' : '跳轉下一章'} <ArrowRight size={16} />
                        </button>
                        <button 
                            onClick={onExit}
                            className={`w-full py-3 border-2 font-bold uppercase tracking-wide transition-all hover:scale-[1.02] border-ash-gray/50 text-ash-gray hover:border-ash-light hover:text-ash-light`}
                        >
