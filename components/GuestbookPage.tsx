...
import { MessageSquare, Send, Radio, Wifi, Database, User, ShieldAlert, Cpu, Terminal, CloudOff, Globe, Server, Loader2, ArrowLeft, Trash2, Lock, Key, Eye, Radar, Music, Activity, X, Crown, History, Calendar, ChevronRight } from 'lucide-react';
...
// Helper: Calculate Segment Info (Spring: 3-6, Summer/Autumn: 7-10, Winter: 11-2)
const getSegmentInfo = (ts: number) => {
    ...
    // Key: 2026-S1
    const key = `${displayYear}-S${segment}`;
    // Label: 2026.03 - 2026.06
    const label = segment === 3 
        ? `${displayYear}.11 - ${displayYear + 1}.02`
        : `${displayYear}.${String(startMonth).padStart(2, '0')} - ${displayYear}.${String(endMonth).padStart(2, '0')}`;
    // Sort Value: 20261
    const sortValue = displayYear * 10 + segment;
    
    return { key, label, sortValue };
};

// Helper: Normalize Message Data from API (Fixes Timestamp Bug)
const normalizeMessage = (msg: any): Message => {
    let timestamp = parseInt(msg.timestamp || 0);
    // FIX: If timestamp is in seconds (e.g. PHP time()), convert to milliseconds
    ...
    return {
        ...
        sender: msg.sender,
        content: msg.content,
        timestamp: timestamp, // Explicitly include timestamp
        // Handle string booleans from DB ('1', 'true')
        isSystem: msg.isSystem == 1 || msg.isSystem === 'true',
        isAdmin: msg.isAdmin == 1 || msg.isAdmin === 'true'
    };
};
...
const GuestbookPage: React.FC<GuestbookPageProps> = ({ 
  ...
  // Filter out internal test messages from UI
  const visibleMessages = useMemo(() => {
      return messages.filter(m => !/^\[\[TEST_MSG::.*\]\]$/.test(m.content));
  }, [messages]);

  // --- TIME SEGMENTATION LOGIC ---
  const segments = useMemo(() => {
      const map = new Map<string, { key: string, label: string, sortValue: number }>();
      
      // Always include current time segment so it's not empty on fresh start
      const currentQ = getSegmentInfo(Date.now());
      map.set(currentQ.key, currentQ);

      visibleMessages.forEach(msg => {
          const info = getSegmentInfo(msg.timestamp);
          if (!map.has(info.key)) {
  ...
  // Initialize active segment to newest available
  useEffect(() => {
      if ((!activeSegment || !segments.find(s => s.key === activeSegment)) && segments.length > 0) {
  ...
  const displayedMessages = useMemo(() => {
      ...
      return visibleMessages
        .filter(m => getSegmentInfo(m.timestamp).key === activeSegment)
        .sort((a, b) => a.timestamp - b.timestamp); // Keep chronological order within segment for chat flow
  }, [visibleMessages, activeSegment]);
  ...
  useEffect(() => {
      ...
      const interval = setInterval(() => {
          if (!isWidget || isOpen) loadMessages(true); // Silent update
      }, 10000);
      return () => clearInterval(interval);
  }, [adminMode, isOpen, isWidget]); 
  ...
  const loadMessages = async (silent = false) => {
      ...
      try {
          ...
          if (Array.isArray(rawData)) {
              // Normalize data here
              const normalizedData = rawData.map(normalizeMessage);
  ...
  // Auto Scroll - Triggered when displayed messages change
  useEffect(() => {
  ...
  }, [displayedMessages.length, viewMode, isOpen, activeSegment]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      if (finalContent.startsWith('/')) {
          ...
          if (firstSpace > 0) {
              ...
              if (cmd === 'alert') {
                  ...
                  finalContent = `[[GLITCH::${payload}]]`;
              } else if (cmd === 'rainbow') {
                  finalContent = `[[RAINBOW::${payload}]]`;
              } else if (cmd === 'term') {
                  finalContent = `[[TERM::${payload}]]`;
              }
          }
      }
      ...
      // Ensure we switch to the latest segment to see our new message
      const currentQ = getSegmentInfo(newMsgObj.timestamp);
      if (activeSegment !== currentQ.key) {
          setActiveSegment(currentQ.key);
      }
  ...
  const renderMessageContent = (content: string, textClass: string) => {
      ...
      if (content.startsWith('[[RADIO::') && content.endsWith(']]')) {
          return (
              <span className="text-cyan-400 flex items-start gap-2">
      ...
      if (content.startsWith('[[LOG::') && content.endsWith(']]')) {
          return (
              <span className="text-gray-500 text-xs block bg-black/10 p-1 border-l-2 border-gray-500">
                  {content.slice(7, -2)}
              </span>
          );
      }
      if (content.startsWith('[[GLITCH::') && content.endsWith(']]')) {
          const text = content.slice(10, -2);
          return (
              <span className="glitch-text-heavy text-red-500 font-black tracking-widest inline-block" data-text={text}>
                  {text}
              </span>
          );
      }
      if (content.startsWith('[[RAINBOW::') && content.endsWith(']]')) {
          return (
              <span className="bg-gradient-to-r from-red-500 via-yellow-500 to-purple-500 bg-clip-text text-transparent animate-pulse font-black tracking-wide inline-block">
                  {content.slice(11, -2)}
              </span>
          );
      }
      if (content.startsWith('[[TERM::') && content.endsWith(']]')) {
          return (
              <div className="bg-black border-2 border-green-500 p-2 font-mono text-xs text-green-500 mt-1 mb-1 shadow-[0_0_10px_rgba(34,197,94,0.3)] block w-full">
                  <div className="border-b border-green-500/50 pb-1 mb-1 font-bold tracking-widest flex items-center gap-2">
                      <Terminal size={10} /> SYSTEM_OVERRIDE
                  </div>
                  <div className="leading-relaxed whitespace-pre-wrap">
                      {content.slice(8, -2)}<span className="animate-[blink_1s_infinite] inline-block w-2 h-4 bg-green-500 align-middle ml-1"></span>
                  </div>
              </div>
          );
      }
  ...
  // --- Main UI Content ---
  const MainContent = (
    <div className={`flex flex-col relative overflow-hidden h-full ${isLightTheme ? 'bg-zinc-50' : 'bg-[#050505]'}`}>
                ...
                >
                    ...
                    <div className="flex items-center gap-3 mb-4 border-b border-dashed pb-2 border-current/30">
                        ...
                        <div>
                            ...
                            <div className="text-[10px] opacity-60">{formatTime(viewingMsg.timestamp)}</div>
                        </div>
                        {adminMode && (
                            <button 
                    ...
                    <div className="mt-6 text-[10px] opacity-40 text-center uppercase">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 text-[10px] md:text-xs opacity-60 mt-1">
                        ...
                        <Database size={12} /> {visibleMessages.length} LOGS
                    </div>
                </div>
            </div>
        ...
        {/* Time Segment Tape (Horizontal Axis) */}
        {segments.length > 0 && (
            <div className={`w-full overflow-x-auto border-b-2 flex items-center shrink-0 no-scrollbar ${isLightTheme ? 'bg-zinc-100 border-zinc-200' : 'bg-[#080808] border-emerald-900/20'}`}>
                <div className="flex px-4 py-2 gap-2 min-w-max">
                    <div className={`text-[10px] font-mono font-bold uppercase py-1 px-2 flex items-center gap-2 ${isLightTheme ? 'text-zinc-400' : 'text-emerald-900'}`}>
                    ...
                    {segments.map((seg) => {
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    text-[10px] md:text-xs font-mono font-bold px-3 py-1 border transition-all uppercase whitespace-nowrap
                                    ${isActive 
                                        ? (isLightTheme 
                                            ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm' 
                                            : 'bg-emerald-900/40 text-emerald-400 border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.2)]')
                                        : (isLightTheme
                                            ? 'bg-white text-zinc-500 border-zinc-300 hover:text-black hover:border-zinc-400'
                                            : 'bg-transparent text-emerald-800 border-emerald-900/30 hover:text-emerald-600 hover:border-emerald-800')
                                    }
                                `}
                            >
                                <span className="opacity-60 mr-1">{seg.key.split('-')[0]}</span>
                                <span>{seg.label.split(' - ')[0].split('.')[1]} - {seg.label.split(' - ')[1].split('.')[1]}</span>
                            </button>
                        );
                    })}
                </div>
            ...
            {viewMode === 'list' && (
                ...
                >
                    ...
                    {!isLoading && displayedMessages.length === 0 && (
                        <div className="flex flex-col items-center justify-center h-full text-ash-gray opacity-50 gap-2">
                            <Calendar size={32} />
                            <div className="text-xs font-mono uppercase tracking-widest">NO_DATA_IN_SECTOR // {activeSegment}</div>
                        </div>
                    )}
            ...
            <form onSubmit={handleSubmit} className="max-w-4xl mx-auto flex flex-col gap-2">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <input 
                        ...
                        className={`bg-transparent border-b text-xs w-32 px-1 focus:outline-none focus:border-emerald-500 transition-colors ${isLightTheme ? 'border-zinc-300 text-zinc-800' : 'border-ash-gray text-ash-light'}`}
                        maxLength={12}
                    />
                    {!isConnected && (
                ...
                <div className="flex gap-2">
                    <div className={`flex-1 flex items-center border-2 transition-colors ${isLightTheme ? 'bg-zinc-50 border-zinc-300 focus-within:border-emerald-500' : 'bg-black border-ash-gray/50 focus-within:border-emerald-500/50'}`}>
                        ...
                        <input 
                            ...
                            className={`w-full bg-transparent border-none p-3 focus:outline-none text-sm ${isLightTheme ? 'text-black placeholder:text-zinc-400' : 'text-ash-white placeholder:text-ash-gray/30'}`}
                        />
                    </div>
                    <button 
                        type="submit"
