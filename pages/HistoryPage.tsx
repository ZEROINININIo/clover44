...
const HistoryPage: React.FC<HistoryPageProps> = ({ onNavigate, isLightTheme = false }) => {
  ...
  return (
    <div className={`p-6 md:p-12 max-w-5xl mx-auto h-full overflow-y-auto font-mono ${isLightTheme ? 'bg-zinc-100 text-zinc-800' : 'bg-ash-black text-white'}`}>
      <header className="mb-8 border-b-2 border-dashed border-ash-gray pb-6 flex justify-between items-end">
        <div>
          <h2 className={`text-2xl font-bold mb-2 flex items-center gap-3 ${isLightTheme ? 'text-zinc-900' : 'text-white'}`}>
            <Terminal size={24} /> 
