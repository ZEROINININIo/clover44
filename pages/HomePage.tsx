...
import { Hexagon, Terminal, Command, RefreshCw, AlertTriangle } from 'lucide-react'; // Keep standard icons for small UI elements
import { NovaTerminal, NovaData, NovaFragments } from '../components/icons/NovaIcons'; // Import custom icons
...
const DigitalHorizon = ({ isLightTheme, quality, mousePos }: { isLightTheme: boolean, quality: GraphicsQuality, mousePos: {x: number, y: number} }) => {
    ...
    useEffect(() => {
        // Low quality: Disable canvas entirely to save CPU
        if (quality === 'low') return;

        const canvas = canvasRef.current;
        if (!canvas) return;
        const ctx = canvas.getContext('2d', { alpha: false }); // Optimize by disabling alpha on main canvas if possible, but we need it since we paint bg. Alpha: false is fine since we paint a solid bg first.
        if (!ctx) return;
        ...
        let cachedHorizonZ = -1;
        let cachedHorizonW = -1;
        let horizonGradient: CanvasGradient;
        
        let floorGradient: CanvasGradient;
        let cachedFloorH = -1;

        const resize = () => {
            w = canvas.width = window.innerWidth;
            h = canvas.height = window.innerHeight;
            cachedHorizonZ = -1; // force gradient rebuild
        };
        window.addEventListener('resize', resize, { passive: true });

        const particleCount = quality === 'medium' ? 60 : 120;
        ...
        const draw = () => {
            ...
            // Adjust perspective center based on mouse position
            const cx = w / 2 - mousePosRef.current.x * 2;
            const cy = h / 2 - mousePosRef.current.y * 2;
            const fov = 300;

            // Cache & Draw Horizon Haze
            if (cy !== cachedHorizonZ || w !== cachedHorizonW) {
                horizonGradient = ctx.createLinearGradient(0, cy - 50, 0, cy + 100);
                horizonGradient.addColorStop(0, "transparent");
                horizonGradient.addColorStop(0.5, horizonColor);
                horizonGradient.addColorStop(1, "transparent");
                
                // Also cache floor gradient
                floorGradient = ctx.createLinearGradient(0, cy + 150, 0, cy);
                floorGradient.addColorStop(0, gridColor);
            ...
            for (let x = -w * 2; x < w * 2; x += gridSize * 1.5) {
            ...
            for (let i = 0; i < particles.length; i++) {
                ...
                const size = Math.max(0, (1 - p.z / 2000) * 2.5);
                
                ctx.globalAlpha = 1 - p.z / 2000;
                // Optimization: replace arc with fillRect
    ...
    }, [isLightTheme, quality]);

    if (quality === 'low') {
        return <div className={`fixed inset-0 z-0 pointer-events-none ${isLightTheme ? 'bg-zinc-100' : 'bg-[#09090b]'}`} />;
    }

    return <canvas ref={canvasRef} className="fixed inset-0 z-0 pointer-events-none" style={{ transform: 'translateZ(0)', willChange: 'transform' }} />;
};

const HomePage: React.FC<HomePageProps> = ({ onNavigate, language, setLanguage, nickname, isLightTheme = false, graphicsQuality }) => {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        // Disable parallax math on mobile devices entirely to save CPU
        if (isMobileRef.current || graphicsQuality === 'low') return;
        if (!containerRef.current) return;
        
        // Use requestAnimationFrame throttle for getBoundingClientRect
    ...
    const parallaxStyle = (graphicsQuality !== 'low' && typeof window !== 'undefined' && window.innerWidth > 768) ? {
    ...
    const themeColors = isLightTheme ? {
        ...
        bg: graphicsQuality === 'low' ? 'bg-white' : 'bg-white/90',
        glass: graphicsQuality === 'low' ? 'bg-white' : 'bg-white/60',
        ...
        bg: graphicsQuality === 'low' ? 'bg-[#09090b]' : 'bg-ash-black/80',
        glass: graphicsQuality === 'low' ? 'bg-[#09090b]' : 'bg-ash-black/40',
        decor: 'border-ash-light/10'
    };

    return (
        <div 
            className={`h-full overflow-y-auto overflow-x-hidden relative flex flex-col items-center perspective-1000 pb-24 md:pb-0 ${isLightTheme ? 'bg-zinc-100' : 'bg-[#09090b]'}`}
            onMouseMove={handleMouseMove}
        >
            <DigitalHorizon isLightTheme={isLightTheme} quality={graphicsQuality} mousePos={mousePos} />
            
            {/* Background Vignette & Scanlines - Disable on Low */}
            {graphicsQuality !== 'low' && (
                <>
            ...
            <div className={`absolute top-4 left-4 md:top-6 md:left-6 z-20 transition-all duration-1000 delay-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
                <div className={`flex items-center gap-2 md:gap-3 ${themeColors.text} font-mono text-xs uppercase tracking-widest`}>
                    <div className={`w-6 h-6 md:w-8 md:h-8 border border-current flex items-center justify-center ${graphicsQuality !== 'low' ? 'animate-spin-slow' : ''}`}>
                        <Hexagon size={14} className="md:w-4 md:h-4" />
                    </div>
                    <div className="flex flex-col">
                        <div className="font-bold items-center gap-2 hidden md:flex">
                            <span className={`w-1.5 h-1.5 bg-emerald-500 rounded-full ${graphicsQuality !== 'low' ? 'animate-pulse' : ''}`}></span>
                            {t.system}
                        </div>
                        <div className="flex items-center gap-2 mt-0.5">
                            <div className={`text-[9px] md:text-[10px] ${themeColors.textMuted}`}>
                                CORE: {currentVersion}
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex flex-col items-center gap-6 md:gap-16 py-12 md:py-0">
                    ...
                    <div className={`text-center space-y-4 md:space-y-6 transition-all duration-1000 ease-out ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-90 blur-sm'}`}>
                        ...
                        <div className="relative w-20 h-20 md:w-32 md:h-32 mx-auto group">
                            {/* Spinning Brackets - Disable on Low */}
                            {graphicsQuality !== 'low' && (
                                <>
                            ...
                            {/* Main Logo Box */}
                            <div className={`absolute inset-0 border-2 ${themeColors.border} ${themeColors.glass} ${graphicsQuality !== 'low' ? 'backdrop-blur-sm' : ''} rotate-45 transition-transform duration-700 group-hover:rotate-0`}></div>
                            ...
                            <img 
                                ...
                                alt="Logo" 
                                className={`absolute inset-0 w-full h-full object-contain p-4 md:p-6 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] select-none ${isLightTheme ? 'filter invert brightness-50' : ''}`}
                        ...
                        <div>
                            <h1 className={`text-3xl md:text-7xl font-black tracking-tighter uppercase mb-2 ${themeColors.text} ${graphicsQuality !== 'low' ? 'glitch-text-heavy' : ''}`} data-text="NOVA LABS">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 w-full max-w-sm md:max-w-4xl">
                        ...
                        <div className={`col-span-2 md:col-span-1 md:col-start-2 md:row-start-1 transition-all duration-1000 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                            <button 
                                ...
                                className={`
                                    ...
                                    border-2 ${themeColors.border} ${themeColors.bg} ${graphicsQuality !== 'low' ? 'backdrop-blur-md' : ''}
                            ...
                            >
                                ...
                                <div className={`p-2 md:p-4 rounded-full border border-current group-hover:bg-emerald-500 group-hover:text-black transition-colors ${isLightTheme ? 'text-zinc-800' : 'text-ash-light'}`}>
                                    <NovaTerminal size={24} className="fill-current md:w-8 md:h-8" />
                                </div>
                        ...
                        <div className={`col-span-1 md:col-start-1 md:row-start-1 flex flex-col justify-center transition-all duration-1000 delay-500 ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
                            <button 
                                ...
                                className={`
                                    ...
                                    border ${themeColors.border} ${themeColors.bg} ${graphicsQuality !== 'low' ? 'backdrop-blur-sm' : ''}
                            ...
                            >
                                ...
                                <NovaData size={18} className={`${themeColors.textMuted} group-hover:text-cyan-500 transition-colors md:w-6 md:h-6`} />
                        ...
                        <div className={`col-span-1 md:col-start-3 md:row-start-1 flex flex-col justify-center transition-all duration-1000 delay-500 ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
                            ...
                            >
                                ...
                                <NovaFragments size={18} className={`${themeColors.textMuted} group-hover:text-fuchsia-500 transition-colors md:w-6 md:h-6`} />
                    ...
                    <div className={`w-full max-w-xs md:max-w-md mx-auto transition-all duration-1000 delay-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        <div className={`
                            ...
                            ${graphicsQuality !== 'low' ? 'backdrop-blur-sm' : ''} overflow-hidden
                        `}>
