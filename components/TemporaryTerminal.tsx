...
const TemporaryTerminal: React.FC<TemporaryTerminalProps> = ({ onClose, onComplete, language, scriptId }) => {
  ...
  return (
    <div className="absolute inset-0 z-[150] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 md:p-8 animate-fade-in font-mono cursor-pointer" onClick={handleInteraction}>
        <div 
                    ...
                    {isSystem ? (
                        ...
                        <div className={`relative w-32 h-32 md:w-40 md:h-40 border ${borderColor} p-1 ${isGlitch || isPain ? 'animate-shake-violent' : ''} transition-all duration-500`}>
                            <img 
                                ...
                                alt="Byaki" 
                                className={`w-full h-full object-cover filter select-none ${isPain ? 'sepia hue-rotate-[-50deg] saturate-200' : isAwakening ? 'brightness-125 contrast-125' : 'sepia hue-rotate-50 contrast-125 saturate-50'} ${isGlitch ? 'opacity-50 blur-[1px]' : 'opacity-90'}`} 
