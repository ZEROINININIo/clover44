...
import { Binary, Database, Brain, CloudRain, Clock, Cpu, GitBranch, Activity, Moon, Radio, Sun, Hourglass, Star, Zap } from 'lucide-react';
...
const SideStoryEntryAnimation: React.FC<SideStoryEntryAnimationProps> = ({ onComplete, language, volumeId }) => {
  ...
  const isGhostAnalysisTheme = volumeId === 'VOL_GHOST_ANALYSIS';
  const isFirstStepTheme = volumeId === 'VOL_FIRST_STEP';
  ...
  const t = {
    'zh-CN': {
      ...
      collab_sync: "接触确认",
      // Ghost Analysis Theme
      ghost_init: "扫描析界节区...",
      ghost_mem: "捕捉幽灵信号：[GHOST]",
      ghost_sync: "数据重组中 // 正在进入析界",
      // First Step Theme
      fs_init: "系统终端翻译器接驳...",
      fs_mem: "回溯记录 // time obj. ST.?",
      fs_sync: "验证权限"
    },
    'zh-TW': {
      accessing: "訪問記憶扇區...",
      ...
      // Ghost Analysis Theme
      ghost_init: "掃描析界節區...",
      ghost_mem: "捕捉幽靈信號：[GHOST]",
      ghost_sync: "數據重組中 // 正在進入析界",
      // First Step Theme
      fs_init: "系統終端翻譯器接駁...",
      fs_mem: "回溯記錄 // time obj. ST.?",
      fs_sync: "驗證權限"
    },
    'en': {
      ...
      // Ghost Analysis Theme
      ghost_init: "SCANNING_ANALYSIS_SECTOR...",
      ghost_mem: "CAPTURING_GHOST_SIGNAL: [GHOST]",
      ghost_sync: "DATA_RECOMBINATION // ENTERING_ANALYSIS",
      // First Step Theme
      fs_init: "SYSTEM_TERMINAL_TRANSLATOR_CONNECT...",
      fs_mem: "RETRIEVING_RECORD // time obj. ST.?",
      fs_sync: "VERIFYING_ACCESS"
    }
  }[language];
            ...
            <div className="relative z-10 flex flex-col items-center gap-12">
                ...
                <div className="relative w-48 h-48 flex items-center justify-center">
                    ...
                    <div className={`transition-all duration-700 absolute ${stage === 3 ? 'scale-125 opacity-100 blur-sm' : 'scale-50 opacity-0'}`}>
                         <StraightStar size={64} className="text-white fill-white/20" />
                    </div>
                </div>
  ...
  // --- Ghost Analysis Theme Render (Glitchy/Ghostly/Slate) ---
  if (isGhostAnalysisTheme) {
      return (
        <div className="fixed inset-0 z-[99999] bg-[#05080a] text-slate-100 overflow-hidden flex flex-col items-center justify-center font-mono cursor-none">
             <style>{`
                @keyframes ghost-glitch {
                    ...
                    80% { transform: translate(2px, 2px); clip-path: inset(20% 0 50% 0); }
            ...
            <button 
                onClick={onComplete}
                className="absolute top-4 right-4 z-[100000] text-[10px] font-mono border border-slate-500/50 text-slate-400 px-2 py-1 hover:bg-slate-500 hover:text-white transition-colors cursor-pointer"
            >
                [DECODE_GHOST]
            </button>

            {/* Background - Glitchy Scanlines */}
            <div className={`absolute inset-0 opacity-20 transition-opacity duration-1000 ${stage > 0 ? 'opacity-40' : 'opacity-0'}`}>
            ...
            <div className="relative z-10 flex flex-col items-center gap-12">
                ...
                {/* Ghostly Icon Container */}
                <div className="relative w-48 h-48 flex items-center justify-center">
                    {/* Pulsing Outer Ring */}
                    <div className={`absolute inset-0 border-2 border-slate-500/20 rounded-full transition-all duration-1000 ${stage >= 1 ? 'scale-110 opacity-100' : 'scale-50 opacity-0'}`}></div>
                    
                    {/* Glitchy Icon */}
                    <div className={`relative transition-all duration-700 ${stage >= 1 ? 'scale-100 opacity-100' : 'scale-50 opacity-0'}`}>
                         <div className={stage === 2 ? 'animate-[ghost-glitch_0.2s_infinite]' : ''}>
                            {stage === 1 && <Activity size={64} className="text-slate-400" />}
                            {stage >= 2 && <Zap size={80} className="text-slate-200 drop-shadow-[0_0_15px_rgba(148,163,184,0.8)]" />}
                         </div>
                    </div>
                ...
                {/* Text Status */}
                <div className="text-center space-y-4 h-24 transition-opacity duration-300">
                    <div className="text-sm md:text-xl font-bold font-mono tracking-[0.4em] text-slate-300 uppercase flex flex-col items-center gap-2">
                        {stage === 0 && ""}
                        {stage === 1 && <span className="text-slate-500 animate-pulse">{t.ghost_init}</span>}
                        {stage === 2 && <span className="text-slate-200">{t.ghost_mem}</span>}
                        {stage === 3 && t.ghost_sync}
                    </div>
                    
                    {/* Glitchy Progress Bar */}
                    <div className="w-48 h-1 bg-slate-900/50 mx-auto overflow-hidden relative border border-slate-800">
                        <div 
                            className="absolute inset-y-0 bg-slate-400 transition-all duration-300 ease-linear shadow-[0_0_10px_rgba(148,163,184,0.5)]"
                            style={{ width: `${(stage / 3) * 100}%`, left: 0 }}
                        ></div>
                    </div>
                </div>
            ...
            {/* Final Flash - Slate White */}
            <div className={`absolute inset-0 bg-[#e2e8f0] pointer-events-none transition-opacity duration-1000 ease-out ${stage === 3 ? 'opacity-100' : 'opacity-0'}`}></div>
        </div>
      );
  }

  // --- First Step Theme Render (Brutalist/Black&White) ---
  if (isFirstStepTheme) {
      return (
        <div className="fixed inset-0 z-[99999] bg-[#f4f4f5] text-black overflow-hidden flex flex-col items-center justify-center font-mono cursor-none">
             {/* Skip */}
            <button 
                onClick={onComplete}
                className="absolute top-4 right-4 z-[100000] text-[10px] font-mono border border-black/50 text-black/80 px-2 py-1 hover:bg-black hover:text-white transition-colors cursor-pointer"
            >
                [FORBIDDEN_SKIP]
            </button>

            {/* Background Glitch Elements */}
            <div className={`absolute inset-0 pointer-events-none opacity-10 transition-opacity duration-1000 ${stage > 0 ? 'opacity-20' : 'opacity-0'}`}>
            ...
            <div className="relative z-10 flex flex-col items-center justify-center">
                 {/* Rotating Geometrics */}
                 <div className="relative w-48 h-48 flex items-center justify-center">
                     <div className={`absolute border-4 border-black transition-all duration-[2000ms] ease-in-out ${stage >= 1 ? 'w-48 h-48 rotate-90 opacity-100' : 'w-0 h-0 rotate-0 opacity-0'}`}></div>
                     <div className={`absolute border border-black transition-all duration-[1500ms] ease-in-out ${stage >= 2 ? 'w-32 h-32 rotate-45 opacity-100' : 'w-64 h-64 rotate-0 opacity-0'}`}></div>
                     
                     <div className={`relative flex items-center justify-center bg-black transition-all duration-700 ${stage >= 1 ? 'w-16 h-16 opacity-100' : 'w-4 h-4 opacity-0'}`}>
                         {/* We import Eye from lucide-react if needed, or we use a simple shape. The file already imports many icons but let's check. Actually Eye is not originally in SideStoryEntryAnimation. Let's use Database or Radio or just a white square. */}
                         <div className={`w-8 h-8 rounded-full border-2 border-white transition-all duration-500 ${stage >= 2 ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}></div>
                         <div className={`absolute w-2 h-2 bg-white rounded-full transition-all duration-500 delay-300 ${stage >= 2 ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}></div>
                     </div>
                 </div>

                 {/* Text Status */}
                 <div className="text-center space-y-4 h-24 mt-12 transition-opacity duration-300">
                     <div className="text-sm md:text-xl font-black font-mono tracking-widest text-black uppercase flex flex-col items-center gap-2">
                         {stage === 0 && ""}
                         {stage === 1 && <span className="animate-pulse">{t.fs_init}</span>}
                         {stage === 2 && <span>{t.fs_mem}</span>}
                         {stage === 3 && <span className="text-red-700 font-bold border-b-2 border-red-700 pb-1">{t.fs_sync}</span>}
                     </div>
                     
                     {/* Progress Bar (Brutalist Segmented) */}
                     <div className="w-64 h-2 bg-gray-300 mx-auto flex relative overflow-hidden">
                         <div 
            ...
            {/* Final Flash - Black */}
            <div className={`absolute inset-0 bg-black pointer-events-none transition-opacity duration-1000 ease-in ${stage === 3 ? 'opacity-100' : 'opacity-0'}`}></div>
        </div>
      );
  }
