...
import { X, FolderOpen, Image as ImageIcon, CornerDownRight, FileWarning, Skull, AlertTriangle, RefreshCw, Bug, Ghost, Binary, Zap } from 'lucide-react';
// Import new minimalist icons
import { NovaDbHub, NovaDbWorld, NovaDbOrg, NovaDbTech, NovaDbSociety, NovaDbSetting } from '../components/icons/NovaIcons';
...
// Standard Functional Nodes with new NovaDb icons
// Moved inside component to support theme-aware colors

const DatabasePage: React.FC<DatabasePageProps> = ({ language, nickname = "TEA", isLightTheme = false }) => {
  // Standard Functional Nodes with new NovaDb icons
  const standardNodes: MapNode[] = useMemo(() => [
      { id: 'All', type: 'Category', category: 'All', x: 0, y: 0, icon: NovaDbHub, color: isLightTheme ? 'text-zinc-800' : 'text-white' },
      { id: 'World', type: 'Category', category: 'World', x: -300, y: -150, icon: NovaDbWorld, color: 'text-cyan-400' },
      { id: 'Organization', type: 'Category', category: 'Organization', x: 300, y: -150, icon: NovaDbOrg, color: 'text-fuchsia-400' },
      { id: 'Technology', type: 'Category', category: 'Technology', x: 300, y: 150, icon: NovaDbTech, color: 'text-emerald-400' },
      { id: 'Society', type: 'Category', category: 'Society', x: -300, y: 150, icon: NovaDbSociety, color: 'text-amber-400' },
      { id: 'Setting', type: 'Category', category: 'Setting', x: 0, y: 350, icon: NovaDbSetting, color: 'text-red-400' },
  ], [isLightTheme]);
  ...
  const allNodes = useMemo(() => {
      ...
      return [...standardNodes, ...generatedCorruptions];
  }, [standardNodes]);
                        ...
                        {filteredLore.map((entry, index) => {
                            ...
                            return (
                                ...
                                <div className="bg-ash-black border-2 border-ash-gray p-4 md:p-8 hover:border-ash-light transition-colors relative group">
                                    ...
                                    <div className={`space-y-4 font-mono text-xs md:text-sm leading-6 md:leading-7 ${isLightTheme ? 'text-zinc-700' : 'text-ash-light'}`}>
                                    {tLore.content.map((para, idx) => {
                                            ...
                                            <div key={idx} className="my-6">
                                                <div className="relative border-2 border-ash-gray p-2 bg-ash-dark/30 inline-block max-w-full">
                                                    <img 
                                                        src={src} 
                                                        alt={caption}
                                                        className="max-w-full h-auto max-h-64 object-contain mx-auto block grayscale-[20%] hover:grayscale-0 transition-all select-none"
                                        ...
                                        return (
                                            <p key={idx}>
                                                {parts.map((part, i) => 
                                                    i % 2 === 1 
                                                    ? <strong key={i} className={`px-1 border border-ash-gray/50 ${isLightTheme ? 'text-zinc-900 bg-zinc-200' : 'text-white bg-ash-dark'}`}>{part}</strong> 
                                                    : part
                                                )}
                                            </p>
                                        )
